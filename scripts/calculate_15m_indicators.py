#!/usr/bin/env python3
"""
ETHUSDT 15-Minute Indicator Calculator
Calculates all 99+ technical indicators for 15m (15m) timeframe
...
INPUT_FILE = DATA_DIR / "ETHUSDT_15m.csv"
FUNDING_FILE = DATA_DIR / "ETHUSDT_funding_rate.csv"
OI_FILE = DATA_DIR / "ETHUSDT_open_interest.csv"
LS_FILE = DATA_DIR / "ETHUSDT_long_short_ratio.csv"
OUTPUT_FILE = DATA_DIR / "ETHUSDT_15m_indicators.csv"
METADATA_FILE = DATA_DIR / "ETHUSDT_15m_metadata.json"
...
def merge_additional_data(df, funding_df, oi_df, ls_df):
    """Merge funding rate, OI, and L/S ratio with 15m candles"""
    ...
    if funding_df is not None and len(funding_df) > 0:
        ...
        for idx, row in df_reset.iterrows():
            ...
            mask = funding_times <= candle_time
            if mask.any():
                last_idx = np.where(mask)[0][-1]
                merged_funding.append(funding_rates[last_idx])
            else:
                merged_funding.append(np.nan)
        
        df_reset['funding_rate'] = merged_funding
        df_reset['funding_extreme_long'] = df_reset['funding_rate'] > 0.001
        df_reset['funding_extreme_short'] = df_reset['funding_rate'] < -0.001
    else:
        df_reset['funding_rate'] = np.nan
        df_reset['funding_extreme_long'] = False
        df_reset['funding_extreme_short'] = False
    ...
    if oi_df is not None and len(oi_df) > 0:
        ...
        for idx, row in df_reset.iterrows():
            candle_time = row['open_time']
            mask = oi_times <= candle_time
            if mask.any():
                last_idx = np.where(mask)[0][-1]
                merged_oi.append(oi_values[last_idx])
            else:
                merged_oi.append(np.nan)
        
        df_reset['open_interest'] = merged_oi
        df_reset['oi_24h_change'] = df_reset['open_interest'].pct_change(96) * 100  # 96 periods = 24h
        df_reset['oi_24h_change'] = df_reset['oi_24h_change'].fillna(0)
        df_reset['oi_rising'] = df_reset['oi_24h_change'] > 0
        df_reset['oi_falling'] = df_reset['oi_24h_change'] < 0
    else:
        df_reset['open_interest'] = np.nan
        df_reset['oi_24h_change'] = 0
        df_reset['oi_rising'] = False
        df_reset['oi_falling'] = False
    
    # Merge Long/Short Ratio
    if ls_df is not None and len(ls_df) > 0:
        ls_times = ls_df['timestamp'].values
        ls_ratios = ls_df['long_short_ratio'].values
        ls_longs = ls_df['long_account_pct'].values
        
        merged_ls = []
        merged_longs = []
        for idx, row in df_reset.iterrows():
            candle_time = row['open_time']
            mask = ls_times <= candle_time
            if mask.any():
                last_idx = np.where(mask)[0][-1]
                merged_ls.append(ls_ratios[last_idx])
                merged_longs.append(ls_longs[last_idx])
            else:
                merged_ls.append(np.nan)
                merged_longs.append(np.nan)
        
        df_reset['long_short_ratio'] = merged_ls
        df_reset['long_account_pct'] = merged_longs
        df_reset['ls_extreme_long'] = df_reset['long_short_ratio'] > 3.0
        df_reset['ls_extreme_short'] = df_reset['long_short_ratio'] < 0.5
        
        conditions = [
            (df_reset['ls_extreme_long'], 'contrarian_short'),
            (df_reset['ls_extreme_short'], 'contrarian_long')
        ]
        df_reset['ls_signal'] = np.select(
            [c[0] for c in conditions],
            [c[1] for c in conditions],
            default='neutral'
        )
    else:
        df_reset['long_short_ratio'] = np.nan
        df_reset['long_account_pct'] = np.nan
        df_reset['ls_extreme_long'] = False
        df_reset['ls_extreme_short'] = False
        df_reset['ls_signal'] = 'neutral'
    
    df_reset.set_index('open_time', inplace=True)
    return df_reset

def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Calculate all technical indicators for 15m data"""
    
    print(f"[{datetime.now()}] Calculating indicators for {len(df)} 15m candles...")
    ...
    # ========== CANDLESTICK PATTERNS (15-Minute Only) ==========
    print(f"[{datetime.now()}]  - Detecting candlestick patterns...")
    pattern_df = detect_candlestick_patterns(df)
    df = pd.concat([df, pattern_df], axis=1)
    print(f"[{datetime.now()}]    ✅ {len(pattern_df.columns)} pattern indicators added")
...
def main():
    """Main function"""
    print("="*70)
    print("ETHUSDT 15-Minute Indicator Calculator")
    print("="*70)
    ...
    print(f"[{datetime.now()}] ✅ Loaded {len(df)} 15m candles")
    
    # Load all additional data sources
    funding_df = load_funding_rate()
    oi_df = load_open_interest()
    ls_df = load_long_short_ratio()
    
    # Calculate indicators
    df = calculate_indicators(df)
    
    # Merge additional data
    df = merge_additional_data(df, funding_df, oi_df, ls_df)
    ...
    metadata = {
        ...
        "indicator_categories": {
            ...
            "candlestick_patterns": [
                ...
                "pattern_spinning_top", "pattern_bullish_engulfing", "pattern_bearish_engulfing",
                "pattern_tweezer_bottom", "pattern_tweezer_top", "pattern_bullish_harami",
                "pattern_bearish_harami", "pattern_morning_star", "pattern_evening_star",
                "pattern_three_white_soldiers", "pattern_three_black_crows",
                "pattern_count", "pattern_bullish_score", "pattern_bearish_score", "pattern_bias"
            ],
            "funding_rate": [
                "funding_rate", "funding_extreme_long", "funding_extreme_short"
            ],
            "open_interest": [
                "open_interest", "oi_24h_change", "oi_rising", "oi_falling"
            ],
            "long_short_ratio": [
                "long_short_ratio", "long_account_pct", "ls_extreme_long",
                "ls_extreme_short", "ls_signal"
            ],
    ...
    print("="*70)
    print(f"Complete! {indicator_count} indicators calculated for {len(df)} 15m candles")
    print("="*70)
