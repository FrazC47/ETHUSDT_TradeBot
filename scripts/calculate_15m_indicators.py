#!/usr/bin/env python3
"""
ETHUSDT 15M Indicator Calculator (Optimized)
Calculates 139 technical indicators for 15m (15-minute) timeframe
Optimized for large datasets - skips slow loop-based trend line calculations
"""
...
INPUT_FILE = DATA_DIR / "ETHUSDT_15m_2022_2024.csv"
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for period in [10, 20]:
        df[f'hist_vol_{period}'] = df['log_returns'].rolling(window=period).std() * np.sqrt(365 * 96) * 100
    ...
    # ========== S/R LEVELS (Vectorized for performance) ==========
    print(f"[{datetime.now()}]  - S/R levels...")
    
    df['resistance_1'] = df['high'].where(df['hh']).ffill()
    df['support_1'] = df['low'].where(df['ll']).ffill()
    ...
    # ========== TREND LINES (Simplified/Vectorized) ==========
    print(f"[{datetime.now()}]  - Trend lines (simplified)...")
    
    # Use rolling regression on swing points (simplified approximation)
    df['bull_trend_line'] = df['low'].rolling(window=50, min_periods=20).min()
    df['bull_trend_valid'] = df['ema_9'] > df['ema_21']
    df['dist_to_bull_trend'] = (df['close'] - df['bull_trend_line']) / df['close'] * 100
    
    df['bear_trend_line'] = df['high'].rolling(window=50, min_periods=20).max()
    df['bear_trend_valid'] = df['ema_9'] < df['ema_21']
...
def main():
    print("="*70)
    print("ETHUSDT 15M Indicator Calculator (Optimized)")
    print("="*70)
