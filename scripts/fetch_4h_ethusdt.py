#!/usr/bin/env python3
"""
ETHUSDT 4-Hour Data Fetcher - Incremental
Only fetches NEW 4h candles since last update
"""
...
CSV_FILE = DATA_DIR / "ETHUSDT_4h.csv"
...
def fetch_new_data(start_time):
    """Fetch only new 4h candles from Binance Futures"""
    ...
    try:
        ...
        if not data:
            print(f"[{datetime.now()}] ℹ️  No new 4h candles available")
            return None
        ...
        print(f"[{datetime.now()}] ✅ Received {len(data)} new 4h candles")
...
def main():
    """Main function"""
    print("="*70)
    print("ETHUSDT 4-Hour Data Update - Incremental Append Mode")
    print("="*70)
    ...
    if data:
        ...
        log_file = DATA_DIR / "4h_update.log"
        with open(log_file, "a") as f:
            f.write(f"{datetime.now().isoformat()} - Appended {count} 4h candles\n")
        
        print(f"[{datetime.now()}] ✅ 4h data update complete ({count} new candles)")
    else:
