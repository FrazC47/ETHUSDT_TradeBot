#!/usr/bin/env python3
"""
ETHUSDT Daily Data Fetcher - Incremental
Only fetches NEW daily candles since last update
"""
...
CSV_FILE = DATA_DIR / "ETHUSDT_1d.csv"
...
def fetch_new_data(start_time):
    """Fetch only new daily candles from Binance Futures"""
    
    params = {
        "symbol": SYMBOL,
        "interval": INTERVAL,
        "startTime": start_time + 1 if start_time > 0 else None,
        "limit": 1000  # Max limit for daily
    }
    ...
    try:
        ...
        if not data:
            print(f"[{datetime.now()}] ℹ️  No new daily candles available")
            return None
        ...
        print(f"[{datetime.now()}] ✅ Received {len(data)} new daily candles")
...
def main():
    """Main function"""
    print("="*70)
    print("ETHUSDT Daily Data Update - Incremental Append Mode")
    print("="*70)
    ...
    if data:
        ...
        with open(log_file, "a") as f:
            f.write(f"{datetime.now().isoformat()} - Appended {count} daily candles\n")
        
        print(f"[{datetime.now()}] ✅ Daily data update complete ({count} new candles)")
    else:
