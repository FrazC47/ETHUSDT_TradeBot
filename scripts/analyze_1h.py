#!/usr/bin/env python3
"""
ETHUSDT 1-Hour Analysis Generator
Analyzes 1h timeframe with 4h + 1d + 1W context as inputs
Saves to: data/analysis/1h_current.json + 1h_log.jsonl
"""
...
INPUT_FILE = DATA_DIR / "ETHUSDT_1h_indicators.csv"
...
def load_parent_contexts():
    """Load 4h, 1d, and 1W analysis as context for 1h analysis"""
    contexts = {
        "four_hour": None,
        "daily": None,
        "weekly": None,
...
def analyze_1h():
    """Generate 1h analysis with 4h + 1d + 1W context"""
    
    if not INPUT_FILE.exists():
        print(f"[{datetime.now()}] ❌ No 1h indicators file found")
        return None
    ...
    # 1h Regime (standalone)
    if ema_bullish and structure_bullish:
        h1_regime = "bullish_accumulation" if rsi < 60 else "bullish_momentum"
    elif not ema_bullish and not structure_bullish:
        h1_regime = "bearish_distribution"
    else:
        h1_regime = "transition"
    ...
    if parent_contexts["loaded_count"] > 0:
        h1_bias = h1_regime.split('_')[0]
        
        # vs 4h
        if parent_contexts["four_hour"]:
            h4_bias = parent_contexts["four_hour"]["bias"]
            if h1_bias == h4_bias:
                alignments["1h_vs_4h"] = "aligned"
                confluence_count += 1
            elif h4_bias == "bullish" and h1_bias == "bearish":
                alignments["1h_vs_4h"] = "1h_pullback_in_bullish_4h"
            elif h4_bias == "bearish" and h1_bias == "bullish":
                alignments["1h_vs_4h"] = "1h_bounce_in_bearish_4h"
            else:
                alignments["1h_vs_4h"] = "mixed"
        else:
            alignments["1h_vs_4h"] = "no_context"
        
        # vs Daily
        if parent_contexts["daily"]:
            daily_bias = parent_contexts["daily"]["bias"]
            if h1_bias == daily_bias:
                alignments["1h_vs_daily"] = "aligned"
                confluence_count += 1
            elif daily_bias == "bullish" and h1_bias == "bearish":
                alignments["1h_vs_daily"] = "1h_pullback_in_bullish_daily"
            elif daily_bias == "bearish" and h1_bias == "bullish":
                alignments["1h_vs_daily"] = "1h_bounce_in_bearish_daily"
            else:
                alignments["1h_vs_daily"] = "mixed"
        else:
            alignments["1h_vs_daily"] = "no_context"
        
        # vs Weekly
        if parent_contexts["weekly"]:
            weekly_bias = parent_contexts["weekly"]["bias"]
            if h1_bias == weekly_bias:
                alignments["1h_vs_weekly"] = "aligned"
                confluence_count += 1
            elif weekly_bias == "bullish" and h1_bias == "bearish":
                alignments["1h_vs_weekly"] = "1h_pullback_in_bullish_weekly"
            elif weekly_bias == "bearish" and h1_bias == "bullish":
                alignments["1h_vs_weekly"] = "1h_bounce_in_bearish_weekly"
            else:
                alignments["1h_vs_weekly"] = "mixed"
        else:
            alignments["1h_vs_weekly"] = "no_context"
        
        # Quad alignment (all 4 timeframes: 1h + 4h + 1d + 1W)
        ...
        alignments = {
            "1h_vs_4h": "no_context",
            "1h_vs_daily": "no_context",
            "1h_vs_weekly": "no_context",
    ...
    analysis = {
        ...
        "interpretations": {
            ...
            "mtf_alignment": {
                ...
                "using_context": ["4h_regime", "daily_regime", "weekly_regime"]
            }
        },
        "comprehensive_analysis": {
            ...
            "narrative": f"1h Analysis: Close {close:.2f}. Regime: {h1_regime}. Quad alignment: {alignments.get('quad_alignment', 'N/A')}. Trend strength: {trend_strength} (ADX {adx:.1f}). Volatility: {vol_regime}. Fibonacci: {fib_zone} ({fib_pos:.2f})."
        }
    }
...
def save_analysis(analysis, candle_date):
    ...
    # Save current state
    current_file = ANALYSIS_DIR / "1h_current.json"
    with open(current_file, 'w') as f:
        json.dump(analysis, f, indent=2)
    
    # Append to log
    log_file = ANALYSIS_DIR / "1h_log.jsonl"
    ...
    if analysis['metadata']['parent_contexts_loaded'] > 0:
        ...
        print(f"  - 1h vs 4h: {alignments.get('1h_vs_4h', 'N/A')}")
        print(f"  - 1h vs Daily: {alignments.get('1h_vs_daily', 'N/A')}")
        print(f"  - 1h vs Weekly: {alignments.get('1h_vs_weekly', 'N/A')}")
        print(f"  - Quad Alignment: {alignments.get('quad_alignment', 'N/A')}")

def main():
    """Main function"""
    print("="*70)
    print("ETHUSDT 1-Hour Analysis Generator (with 4h + 1d + 1W context)")
    print("="*70)
