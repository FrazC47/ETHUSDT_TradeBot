#!/usr/bin/env python3
"""
ETHUSDT 1-Hour Indicator Calculator
Calculates all 99+ technical indicators for 1h (1h) timeframe
...
INPUT_FILE = DATA_DIR / "ETHUSDT_1h.csv"
...
OUTPUT_FILE = DATA_DIR / "ETHUSDT_1h_indicators.csv"
METADATA_FILE = DATA_DIR / "ETHUSDT_1h_metadata.json"
...
def merge_additional_data(df, funding_df, oi_df, ls_df):
    """Merge funding rate, OI, and L/S ratio with 1h candles"""
    ...
    if oi_df is not None and len(oi_df) > 0:
        ...
        df_reset['oi_24h_change'] = df_reset['open_interest'].pct_change(96) * 100  # 24 periods = 24h
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Calculate all technical indicators for 1h data"""
    
    print(f"[{datetime.now()}] Calculating indicators for {len(df)} 1h candles...")
    ...
    # ========== CANDLESTICK PATTERNS (1-Hour Only) ==========
...
def main():
    """Main function"""
    print("="*70)
    print("ETHUSDT 1-Hour Indicator Calculator")
    print("="*70)
    ...
    print(f"[{datetime.now()}] ✅ Loaded {len(df)} 1h candles")
    ...
    print("="*70)
    print(f"Complete! {indicator_count} indicators calculated for {len(df)} 1h candles")
    print("="*70)
