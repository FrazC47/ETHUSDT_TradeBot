#!/usr/bin/env python3
"""
ETHUSDT 1-Hour Data and Indicators Update Script
获取 ETHUSDT 1小时K线数据并计算技术指标
"""
...
LOG_FILE = DATA_DIR / "update_1h.log"
DATA_FILE = DATA_DIR / "ethusdt_1h.csv"
METRICS_FILE = DATA_DIR / "ethusdt_1h_metrics.json"
...
def save_metrics(df, signals):
    ...
    try:
        ...
        metrics = {
            ...
            'price_change_1h': round(float(latest['price_change_1h']), 4) if not pd.isna(latest['price_change_1h']) else None,
            'price_change_4h': round(float(latest['price_change_4h']), 4) if not pd.isna(latest['price_change_4h']) else None,
            'price_change_24h': round(float(latest['price_change_24h']), 4) if not pd.isna(latest['price_change_24h']) else None,
            'indicators': {
                ...
                'rsi_7': round(float(latest['rsi_7']), 2) if not pd.isna(latest['rsi_7']) else None,
            ...
            'volume': {
                ...
                'sma_20': round(float(latest['volume_sma_20']), 4) if not pd.isna(latest['volume_sma_20']) else None,
...
def main():
    """主函数"""
    log_message("=" * 50)
    log_message("Starting ETHUSDT 1-Hour Data Update")
    log_message("=" * 50)
    
    # 1. 获取数据
    log_message("Fetching K-line data from Binance...")
    df = fetch_klines()
    ...
    log_message(f"Fetched {len(df)} data points")
    ...
    if metrics:
        ...
        log_message(f"1H Change: {metrics['price_change_1h']}%")
