#!/usr/bin/env python3
"""
ETHUSDT Weekly Data Fetcher - Incremental
Only fetches NEW weekly candles since last update
"""
...
CSV_FILE = DATA_DIR / "ETHUSDT_1w.csv"
...
def fetch_new_data(start_time):
    """Fetch only new weekly candles from Binance Futures"""
    ...
    try:
        ...
        if not data:
            print(f"[{datetime.now()}] ℹ️  No new weekly candles available")
            return None
        ...
        print(f"[{datetime.now()}] ✅ Received {len(data)} new weekly candles")
...
def main():
    """Main function"""
    print("="*70)
    print("ETHUSDT Weekly Data Update - Incremental Append Mode")
    print("="*70)
    ...
    if data:
        ...
        log_file = DATA_DIR / "weekly_update.log"
        with open(log_file, "a") as f:
            f.write(f"{datetime.now().isoformat()} - Appended {count} weekly candles\n")
        
        print(f"[{datetime.now()}] ✅ Weekly data update complete ({count} new candles)")
    else:
